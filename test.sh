...
cp /environment/yum.repos.d/* /etc/yum.repos.d/
echo "##################配置完成###########################"
sleep 3
echo "##################更新配置###########################"
yum clean all
...
mv /environment/keystone-admin-pass.sh /server/tools/
mv /environment/keystone-myuser-pass.sh /server/tools/
